...
export async function ensureSchema(sql){
  ...
  await sql.query("ALTER TABLE gd_access_control ADD COLUMN IF NOT EXISTS trial_started_at TIMESTAMPTZ");
  await sql.query("ALTER TABLE gd_access_control ADD COLUMN IF NOT EXISTS trial_expires_at TIMESTAMPTZ");
  ...
  await sql.query("CREATE TABLE IF NOT EXISTS gd_driver_line_bindings (owner_id TEXT NOT NULL, driver_id TEXT NOT NULL, line_user_id TEXT NOT NULL, enabled BOOLEAN NOT NULL DEFAULT TRUE, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), PRIMARY KEY(owner_id,driver_id))");
  await sql.query("CREATE INDEX IF NOT EXISTS gd_driver_line_bindings_line_idx ON gd_driver_line_bindings(line_user_id,enabled)");
  await sql.query("CREATE TABLE IF NOT EXISTS gd_system_alerts (id BIGSERIAL PRIMARY KEY, owner_id TEXT NOT NULL, trip_id TEXT, alert_type TEXT NOT NULL, severity TEXT NOT NULL DEFAULT 'warning', status TEXT NOT NULL DEFAULT 'open', message TEXT NOT NULL, payload JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), resolved_at TIMESTAMPTZ)");
  await sql.query("CREATE INDEX IF NOT EXISTS gd_system_alerts_owner_status_idx ON gd_system_alerts(owner_id,status,created_at DESC)");
  await sql.query("CREATE UNIQUE INDEX IF NOT EXISTS gd_system_alerts_open_dedupe_idx ON gd_system_alerts(owner_id,COALESCE(trip_id,''),alert_type) WHERE status='open'");
  await sql.query("CREATE TABLE IF NOT EXISTS gd_admin_notification_bindings (owner_id TEXT PRIMARY KEY, line_user_id TEXT, push_enabled BOOLEAN NOT NULL DEFAULT TRUE, line_enabled BOOLEAN NOT NULL DEFAULT TRUE, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())");
