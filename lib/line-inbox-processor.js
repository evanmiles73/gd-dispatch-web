...
export async function processPendingLineInbox(limit=100){
 ...
 const rows=await sql.query("WITH picked AS (SELECT id FROM gd_line_inbox WHERE processed=false AND source='line_group' ORDER BY id ASC LIMIT $1 FOR UPDATE SKIP LOCKED) UPDATE gd_line_inbox i SET processed=true FROM picked WHERE i.id=picked.id RETURNING i.id,i.payload",[safeLimit]);let matched=0,queued=0,processed=0,failed=0;
