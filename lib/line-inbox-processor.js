...
export async function processPendingLineInbox(limit=100){
 ...
 const rows=await sql.query("SELECT id,payload FROM gd_line_inbox WHERE processed=false AND source='line_group' ORDER BY id ASC LIMIT $1",[limit]);let matched=0,queued=0;
 for(const row of rows){const r=await processLineInboxRow(sql,row);matched+=r.matched;queued+=r.queued}
