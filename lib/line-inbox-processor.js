...
export async function processLineInboxRow(sql,row){
 ...
 if(!prefs.length){
  try{
   // Seed from GD Car's own owner registry. Neon Auth users are not stored in a local "user" table.
   // Access-control rows are created/updated by GD Car and give us stable owner IDs for notifications.
   await sql.query("INSERT INTO gd_grab_preferences(owner_id,enabled,regions,airports,min_amount,keywords,updated_at) SELECT DISTINCT owner_id,true,'[]'::jsonb,'[]'::jsonb,0,'[]'::jsonb,NOW() FROM gd_access_control WHERE owner_id IS NOT NULL AND owner_id<>'' AND status<>'suspended' ON CONFLICT(owner_id) DO NOTHING");
