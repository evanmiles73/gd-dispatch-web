...
export async function processLineInboxRow(sql,row){
 ...
 if(!prefs.length){
  try{
   await sql.query("INSERT INTO gd_grab_preferences(owner_id,enabled,regions,airports,min_amount,keywords,updated_at) SELECT DISTINCT id::text,true,'[]'::jsonb,'[]'::jsonb,0,'[]'::jsonb,NOW() FROM \"user\" WHERE id IS NOT NULL ON CONFLICT(owner_id) DO NOTHING");
