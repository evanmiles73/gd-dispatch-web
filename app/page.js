...
const empty={date:"",time:"",name:"",phone:"",pickup:"",dropoff:"",flight:"",passengers:"",luggage:"",carryOn:"",vehicle:"",amount:"",notes:"",driver:"",driverPhone:""};
...
export default function Home(){
 const [raw,setRaw]=useState(""),[data,setData]=useState(empty),[status,setStatus]=useState("未派"),[trips,setTrips]=useState([]),[tab,setTab]=useState("總行程"),[editing,setEditing]=useState(null);
 ...
 return <main style={{maxWidth:980,margin:"20px auto",padding:16,fontFamily:"system-ui"}}>
  <h1>GD 車趟管理 <small style={{fontSize:14}}>v1.2</small></h1>
  ...
  <section style={card}><h2>新增／確認車趟</h2><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:12}}>
   ...
   {field("carryOn","手提行李（件數）","number")}{field("vehicle","車型")}{field("amount","金額","number")}{field("driver","司機姓名")}{field("driverPhone","司機電話")}{field("notes","備註")}
  ...
  <section style={card}><h2>{tab}</h2>{visible.length===0?<p>目前沒有資料</p>:visible.map(t=><div key={t.id} style={{borderTop:"1px solid #ddd",padding:"12px 0"}}><b>{t.date} {t.time}｜{t.name||"未填姓名"}｜{t.status}</b><div><a href={mapUrl(t.pickup)} target="_blank">📍 {t.pickup||"未填上車地"}</a> → <a href={mapUrl(t.dropoff)} target="_blank">📍 {t.dropoff||"未填下車地"}</a></div><div>航班：{t.flight||"-"}　行李：{t.luggage||"-"}　手提：{t.carryOn||"-"}　金額：{t.amount||"-"}</div><div>司機：{t.driver||"未派"} {t.driverPhone||""}</div><div style={{display:"flex",gap:8,marginTop:8}}><button onClick={()=>edit(t)} style={btn(false)}>編輯</button><button onClick={()=>remove(t.id)} style={btn(false)}>刪除</button></div></div>)}</section>
 </main>
}
