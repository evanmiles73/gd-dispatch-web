...
async function run(){
 ...
 try{
  ...
  for(const job of jobs){
   ...
   if(state==="failed")await sql.query("INSERT INTO gd_system_alerts(owner_id,trip_id,alert_type,severity,message,payload) VALUES($1,$2,'push_delivery_failed','critical',$3,$4::jsonb) ON CONFLICT DO NOTHING",[job.recipient_id,job.trip_id,"推播通知重試 3 次仍失敗",JSON.stringify({outboxId:job.id,lastError,title:job.title})]);
  }
