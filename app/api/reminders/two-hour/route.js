...
async function run(req){
 ...
 try{
  ...
  const rows=await sql.query("SELECT t.id,t.owner_id,t.payload,b.line_user_id FROM gd_trips t JOIN gd_driver_line_bindings b ON b.owner_id=t.owner_id AND b.enabled=TRUE AND (b.driver_id=t.payload->>'driverId' OR b.driver_id=t.payload->>'grabClaimedBy') WHERE COALESCE(t.payload->>'status','') IN ('已接','已派') LIMIT 100");
  const now=Date.now();let sent=0,skipped=0,failed=0,customSent=0;
  ...
  return NextResponse.json({ok:true,checked:rows.length,sent,customSent,skipped,failed});
