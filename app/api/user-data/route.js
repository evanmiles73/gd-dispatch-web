...
async function accessFor(sql,u){const id=u?.id;if(!id)return {role:"free",pricing:false};const email=String(u.email||"").trim().toLowerCase();const configured=(process.env.GD_OWNER_EMAIL||"").split(",").map(x=>x.trim().toLowerCase()).filter(Boolean);if(ROOT_OWNERS.has(email)||configured.includes(email))return {role:"owner",pricing:true};const rows=await sql.query("SELECT role,pricing_enabled,status,pro_expires_at FROM gd_access_control WHERE owner_id=$1",[id]);const a=rows[0];if(!a||a.status==="suspended")return {role:"free",pricing:false};if(a.role==="pro"&&a.pro_expires_at&&new Date(a.pro_expires_at)<=new Date())return {role:"free",pricing:false,expired:true};const role=a.role||"free";return {role,pricing:Boolean(a.pricing_enabled)&&["admin","pro"].includes(role)}}
