...
import {requireWrite} from "../../../lib/access-control";
...
export async function PUT(req){
 ...
 const sql=getDb();if(!sql)return NextResponse.json({ok:false,error:"database_not_configured"},{status:503});
 try{
  await ensureSchema(sql);const gate=await requireWrite(sql,user);if(!gate.ok)return NextResponse.json({ok:false,error:gate.error,role:gate.access.role},{status:403});
...
export async function DELETE(req){
 ...
 try{
  ...
  await sql.query("UPDATE gd_driver_line_bindings SET enabled=false,updated_at=NOW() WHERE owner_id=$1 AND driver_id=$2",[String(user.id),driverId]);
