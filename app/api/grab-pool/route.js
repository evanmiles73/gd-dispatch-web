...
async function canGrab(sql,u){const email=String(u?.email||"").trim().toLowerCase();const configured=(process.env.GD_OWNER_EMAIL||"").split(",").map(x=>x.trim().toLowerCase()).filter(Boolean);if(ROOT_OWNERS.has(email)||configured.includes(email))return true;const rows=await sql.query("SELECT role,status,pro_expires_at,trial_expires_at FROM gd_access_control WHERE owner_id=$1",[u.id]);const a=rows[0];if(!a||a.status==="suspended")return false;if(a.role==="free"&&a.trial_expires_at&&new Date(a.trial_expires_at)>new Date())return true;if(a.role==="pro"&&a.pro_expires_at&&new Date(a.pro_expires_at)<=new Date())return false;return ["pro","admin"].includes(a.role)}
